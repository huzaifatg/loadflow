...
import type { Delivery, DeliveryItem as PrismaDeliveryItem } from '@prisma/client';
...
type ClientDeliveryItem = Omit<PrismaDeliveryItem, 'quantity' | 'unitWeight' | 'totalWeight'> & {
  quantity: number;
  unitWeight: number | null;
  totalWeight: number;
};

type DeliveryWithItems = Omit<Delivery, 'weight'> & {
  weight: number;
  items?: ClientDeliveryItem[];
};

export function DeliveryDetailClient({ delivery }: { delivery: DeliveryWithItems }) {
  const router = useRouter();
  const [isEditing, setIsEditing] = useState(false);
  const [loading, setLoading] = useState(false);

  const deliveryItems = delivery.items || [];
  const hasItems = deliveryItems.length > 0;
  const weightValue = toNumber(delivery.weight);
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      const hasEditItems = editItems.length > 0;
      ...
      if (hasEditItems || hasItems) {
        payload.items = editItems.map((item, i) => ({
          productName: item.productName,
          sku: item.sku || null,
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 border-b border-gray-100 gap-2">
              ...
              {isEditing && !editItems.length ? (
                ...
                <span className="font-medium text-gray-900 text-right">
                  ...
                  {hasItems && <span className="text-xs text-gray-400 ml-1">(from items)</span>}
                </span>
              )}
            </div>
